...
function scorePhase2(board: Board, t: EndgameTuning): number {
  const mx = 6;
  ...
  return (
    ...
    100 * rebuild -
    120 * trapped +
    t.highLevelMergePathWeight * highLevelMergePathValue(board) +
    t.highLevelCornerWeight * maxTileAtAnyCorner(board)
  );
}

/** Phase 3: maxTile ≥ 7 — 타일 분포·갭·재압축·트랩만 (튜닝 가중). */
...
export function scoreBoardV3(board: Board, tuning?: EndgameTuningConfig | null): number {
  ...
  if (mx === 6) return scorePhase2(board, t);
