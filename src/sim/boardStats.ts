...
import type { EndgameTuningConfig } from "./endgameTuning";
...
/** 3×3 보드의 네 구석 (row-major). */
export const CORNER_CELL_INDICES: readonly number[] = [0, 2, 6, 8];

/**
 * 전역 최댓값 레벨을 가진 칸이 네 구석 중 하나에 있으면 1, 아니면 0.
 * (여러 개가 최댓값이면 하나라도 구석이면 만족.)
 */
export function maxTileAtAnyCorner(board: Board): number {
  const mx = maxTileLevel(board);
  if (mx === 0) return 0;
  for (const i of CORNER_CELL_INDICES) {
    if (board[i] === mx) return 1;
  }
  return 0;
}

/**
 * `mergePotentialAtLevel`을 6/7/8에 대해 가중 합산 (전역 max 레벨에 따라 항만 켬).
 * 고레벨 머지로 이어질 수 있는 경로를 점수에 반영.
 */
export function highLevelMergePathValue(board: Board): number {
  const mx = maxTileLevel(board);
  let v = 0;
  if (mx >= 6) v += mergePotentialAtLevel(board, 6) * 1;
  if (mx >= 7) v += mergePotentialAtLevel(board, 7) * 1.35;
  if (mx >= 8) v += mergePotentialAtLevel(board, 8) * 1.65;
  return v;
}
